...
do
    folder=${folder%".txt"}
	echo "${green}Running our treewidth-based approach for computing Markov Chain Hitting Probabilities on benchmark --------> $folder ${reset}"
	...
	do
		...
		./${path}build/mc-hitprob mcs-$folder/$file > mcs-$folder/$file.log
