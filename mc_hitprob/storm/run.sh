...
do
	...
	folder=${folder%".txt"}
	echo "${green}Running Storm for computing Markov Chain Hitting Probabilities on benchmark --------> $folder ${reset}"
    ...
    do
	    ...
	    ${path}/storm/build/bin/storm --prism model.txt --prop "P=? [F x=$target]" > ans.txt
	    sleep 5
