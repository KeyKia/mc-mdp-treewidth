...
int main()
{
    ...
    while ((de = readdir(dr)) != NULL)
    {
        ...
        printf("%s\n", de->d_name);
        if (de->d_name[5] == 'F' && de->d_name[6] == 'r' )
            printf("be to che\n");
        num_mdps++;
        char file_path[256];
...
//        clock_t begin = clock();
        do
        {
            ...
            input(file_path); // read input here each time to re-new everything
            ...
            memset(hitPr, 0, sizeof(hitPr));
            time_spent += solve_mc(bags, bagsNum, e, totV, totE, target, hitPr);
...
//        time_spent += (float) (end - begin) / CLOCKS_PER_SEC;
    ...
    printf("Mean of number of iterations: %f\n", tot_iters/num_mdps);
