...
for filename in ../../benchmarks/dacapobenchmark/outputs/raw_outputs/*.txt; do
	...
	mkdir mcs-$name
	python ../../utils/mcdiscsum_reformat.py ../../benchmarks/dacapobenchmark/outputs/raw_outputs/$name.txt mcs-$name
	totaltime=0
	for txtfile in mcs-$name/*.txt; do
		echo textfile is $txtfile
		./cnv $txtfile >> mcs-$name/matlabcode.m
	done
	../../MATLAB_R2019b.app/bin/matlab -nodisplay -nosplash -nodesktop -r "run('mcs-$name/matlabcode.m');exit;" 2>> mcs-$name/log-matlab.txt
