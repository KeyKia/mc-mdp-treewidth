...
do
    folder=${folder%".txt"}
	echo "${green}Running Gaussian Elimination for computing Markov Chain Discounted Sums of Rewards on benchmark --------> $folder ${reset}"
	...
	do
		...
		./${path}build/gauss-mc-discsum mcs-$folder/$file > mcs-$folder/$file.log
