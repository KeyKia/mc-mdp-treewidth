...
do
    folder=${folder%".txt"}
	echo "${green}Running our treewidth-based approach for computing Markov Chain Discounted Sums of Rewards on benchmark --------> $folder ${reset}"
	percent=`echo "100 * ${cnt} / ${tot}" | bc`

	mkdir mcs-$folder

	python3 ${path}utils/mcdiscsum_reformat.py ${path}benchmarks/dacapobenchmark/outputs/raw_outputs/$folder.txt mcs-$folder
	totaltime=0
	for file in `ls mcs-$folder`
	do
        echo " [${percent}%] ${green} Ours: ${yellow} file ---> ${file} ${reset}"
		./${path}build/mc-discsum mcs-$folder/$file > mcs-$folder/$file.log
