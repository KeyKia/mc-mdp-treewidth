...
bool has_solution(float Ap[][MAX_TREEWIDTH+1]){
    float A[MAX_TREEWIDTH][MAX_TREEWIDTH+1];
    int i, j, k;
...
vector k_independent(vector *conEq, bag *b)
{
    ...
    int i, j, k;
    int first=*(int *)vector_get(conEq,0);
    ...
    for(i=1;i<vector_total(conEq);i++)
    {
        ...
        *ind=*(int *)vector_get(conEq,i);
        ...
        for (j=0; j<vector_total(eqInd+*ind); j++)
        {
            ...
            A[index][varCnt]=*(float *)vector_get(eqCoef+*ind,j);
        }
