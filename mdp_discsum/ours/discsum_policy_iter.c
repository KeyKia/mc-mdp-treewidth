...
#include "../../mc_discsum/ours/mc_discsum_solve.h"
...
int main(int argc, char* argv[])
{
    ...
    do
    {
        ...
        input(argv[1], bags, e, choices, choices_copy, chooseVertices); // re-new everything
        ...
        for (i = 0; i < vector_total(chooseVertices); i++)
        {
            ...
            for (j = 0; j < vector_total(choices_copy + *v); j++)
            {
                ...
                //threshold for floating-point erros
                if (cur->reward + lambda * discSum[cur->u] - maxDiscSum > 1.5)
                {
